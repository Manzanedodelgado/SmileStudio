...
const Agenda: React.FC<AgendaProps> = ({ activeSubArea, initialCita }) => {
        ...
        switch (action) {
            ...
            case 'whatsapp': {
                ...
                if (isEvolutionConfigured()) {
                    ...
                    window.open(`https://wa.me/${tel.replace(/\D/g, '')}?text=${encodeURIComponent(txt)}`, '_blank');
                }
                break;
            }
        }
    ...
    // Renderización de una cita como elemento grid posicionado
    const renderCita = (cita: typeof citas[0], gab: 'G1' | 'G2') => {
        const startRow = timeToGridRow(cita.horaInicio);
        const spanRows = Math.max(1, Math.round(cita.duracionMinutos / 15));
        const overlap = colAssignment.get(cita.id);
        const isOverlapping = !!(overlap && overlap.totalCols > 1);
        const isContacto = cita.pacienteNumPac?.startsWith('CTX-');

        const widthStyle = isOverlapping
            ? { width: `${100 / overlap!.totalCols}%`, left: `${(overlap!.col * 100) / overlap!.totalCols}%`, position: 'absolute' as const, top: 0, bottom: 0 }
            : { left: '4px', right: '4px', position: 'absolute' as const, top: '1px', bottom: '1px' };

        // gridCol viene del _gridCol que le pasamos desde el grid (col 2 para G1, col 3 para G2 en vista ALL)
        const gridCol = (cita as any)._gridCol ?? (gab === 'G1' ? '2' : vistaGabinete === 'ALL' ? '3' : '2');

        // ── Colores: hora en punto → gris/azul con hover. Resto → azul/blanco fijo sin hover ──
        const isSelected = editingCita?.id === cita.id;
        const isPrimeraVis = cita.tratamiento === 'Primera Visita';
        const isOnHour = cita.horaInicio.slice(-1) === '0';
        const bgColor = isSelected ? '#1d4ed8' : isOnHour ? '#f1f3f5' : '#1d4ed8';
        const textColor = isSelected ? '#ffffff' : isOnHour ? '#1d4ed8' : '#ffffff';
        const textSecondary = isSelected ? 'rgba(255,255,255,0.75)' : isOnHour ? 'rgba(29,78,216,0.6)' : 'rgba(255,255,255,0.75)';
        const badgeBg = isSelected ? 'rgba(255,255,255,0.2)' : isOnHour ? 'rgba(29,78,216,0.12)' : 'rgba(255,255,255,0.2)';
        const badgeText = isSelected ? '#ffffff' : isOnHour ? '#1d4ed8' : '#ffffff';
        const badgeColor = isOnHour ? '#1d4ed8' : '#ffffff';
        const badgeBgColor = isOnHour ? '#ffffff' : 'rgba(255,255,255,0.2)';
        const badgeBorderColor = isOnHour ? '#dbeafe' : 'rgba(255,255,255,0.3)';
        const borderLeftColor = isSelected ? '#1e40af' : isOnHour ? '#d0d5dd' : '#1e40af';
        const boxShadowVal = isSelected ? '0 2px 6px rgba(29,78,216,0.3)' : isOnHour ? '0 1px 2px rgba(0,0,0,0.08)' : '0 2px 6px rgba(29,78,216,0.3)';
            ...
            return (
                <div key={cita.id}
                    ...
                    <div style={{
                        ...
                        <span style={{ fontSize: '12px', fontWeight: 700, color: '#9aa0a6', textTransform: 'uppercase', letterSpacing: '0.1em' }}>bioseguridad</span>
                    </div>
                </div>
            );
        }
        ...
        return (
            ...
            >
                <div
                    draggable
                    onClick={e => { e.stopPropagation(); setEditingCita(cita); }}
                    onContextMenu={e => { e.preventDefault(); e.stopPropagation(); setContextMenu({ x: e.pageX, y: e.pageY, cita }); }}
                    onDragStart={e => { e.dataTransfer?.setData('text/plain', cita.id); (e.target as HTMLElement).style.opacity = '0.5'; }}
                    onDragEnd={e => { (e.target as HTMLElement).style.opacity = cita.estado === 'finalizada' ? '0.65' : '1'; }}
                    style={{
                        ...widthStyle,
                        cursor: 'grab',
                        overflow: 'hidden',
                        ...
                        opacity: cita.estado === 'finalizada' ? 0.65 : 1,
                        transition: 'all 150ms',
                        pointerEvents: 'auto',
                        margin: isOverlapping ? '1px 2px' : '1px 4px',
                    }}>
                    {/* Contenido por tramos: primer tramo normal, resto gris tenue */}
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
                        {/* Tramo 1 — contenido principal — misma altura fija siempre */}
                        <div style={{ display: 'flex', alignItems: 'center', padding: '0 6px', overflow: 'hidden', gap: 5, height: '24px', flex: '0 0 24px' }}>
                            {/* NumPac — espacio fijo para alineación */}
                            <span style={{ fontSize: '13px', fontWeight: 700, minWidth: 42, width: 42, textAlign: 'center', padding: '1px 4px', borderRadius: 4, color: cita.pacienteNumPac ? badgeText : 'transparent', background: cita.pacienteNumPac ? badgeBg : 'transparent', whiteSpace: 'nowrap', flexShrink: 0, lineHeight: 1.4 }}>{cita.pacienteNumPac || '\u00A0'}</span>
                            {/* Nombre */}
                            <span style={{ fontSize: '13px', fontWeight: 700, color: textColor, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 1, minWidth: 0 }}>{cita.nombrePaciente?.replace('👤 ', '') || 'Sin datos'}</span>
                            <span style={{ fontSize: '13px', color: textSecondary, flexShrink: 0 }}>·</span>
                            {/* Tratamiento */}
                            <span style={{ fontSize: '12px', fontWeight: 500, color: textSecondary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 2, minWidth: 0 }}>{cita.tratamiento || ''}</span>
                            {/* Notas */}
                            {cita.notas && (
                                <span style={{ fontSize: '12px', fontWeight: 400, fontStyle: 'italic', color: textSecondary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 3, minWidth: 0 }}>— {cita.notas}</span>
                            )}
                            {/* Spacer */}
                            <span style={{ flex: 1, minWidth: 4 }} />
                            {/* Estado — 3 letras, hover muestra completo */}
                            <span data-estado="1" title={(cita.estado?.replace('_', ' ') || '').toUpperCase()} style={{ fontSize: '12px', fontWeight: 700, padding: '1px 4px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap', flexShrink: 0, color: badgeColor, background: badgeBgColor, lineHeight: 1.5, width: 36, minWidth: 36, textAlign: 'center', display: 'inline-block', border: `1px solid ${badgeBorderColor}`, cursor: 'default' }}>{(cita.estado?.replace('_', ' ') || '').substring(0, 3).toUpperCase()}</span>
                            {/* Duración */}
                            <span style={{ fontSize: '12px', fontWeight: 600, color: textSecondary, whiteSpace: 'nowrap', flexShrink: 0, width: 28, minWidth: 28, textAlign: 'right' }}>{cita.duracionMinutos}'</span>
                        </div>
                        {/* Tramos 2+ — réplica en gris tenue */}
                        {Array.from({ length: spanRows - 1 }, (_, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', padding: '0 6px', overflow: 'hidden', gap: 5, flex: '0 0 24px', height: '24px', borderTop: '1px solid rgba(0,0,0,0.04)' }}>
                                <span data-replica="1" style={{ fontSize: '13px', fontWeight: 700, minWidth: 42, width: 42, textAlign: 'center', padding: '1px 4px', borderRadius: 4, color: 'transparent', background: 'transparent', whiteSpace: 'nowrap', flexShrink: 0, lineHeight: 1.4 }}>{cita.pacienteNumPac || '\u00A0'}</span>
                                <span data-replica="1" style={{ fontSize: '13px', fontWeight: 600, color: isSelected ? 'rgba(255,255,255,0.3)' : '#b0b8c4', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 1, minWidth: 0 }}>{cita.nombrePaciente?.replace('👤 ', '') || ''}</span>
                                <span data-replica="1" style={{ fontSize: '13px', color: isSelected ? 'rgba(255,255,255,0.2)' : '#c8ced6', flexShrink: 0 }}>·</span>
                                <span data-replica="1" style={{ fontSize: '12px', fontWeight: 500, color: isSelected ? 'rgba(255,255,255,0.2)' : '#c8ced6', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 2, minWidth: 0 }}>{cita.tratamiento || ''}</span>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    {(() => {
                        ...
                        return (
                            <div data-grid style={{
                                ...
                                gridTemplateRows: `48px repeat(${totalRows}, ${rowH}px)`,
                                gap: '1px',           // ← las líneas SON el gap
                                background: '#e8eaed', // ← color de las líneas
                                width: '100%',
                            }}>
                                {/* ═══ HEADER row 1 ═══════════════════════════════════════════
                                    En la imagen: los nombres de gabinete abarcan toda la anchura
                                    de su mitad — la col de hora no tiene cabecera propia.
                                    En vista ALL: G1 ocupa col 1+2 (span 2), G2 ocupa col 3.
                                    En vista G1/G2: G1 o G2 ocupa col 1+2 (span 2).
                                ═══════════════════════════════════════════════════════════════ */}
                                {vistaGabinete === 'ALL' ? (<>
                                    {/* G1: ocupa col 1 (hora) + col 2 (contenido) */}
                                    <div style={{
                                        ...
                                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#3c4043' }}>
                                            {(() => { const docs = [...new Set(citas.filter(c => c.gabinete === 'G1').map(c => c.doctor))]; return docs.length > 0 ? docs.join(' · ') : 'Gabinete 1'; })()}
                                        </span>
                                    </div>
                                    {/* G2: col 3 */}
                                    <div style={{
                                        ...
                                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#3c4043' }}>
                                            {(() => { const docs = [...new Set(citas.filter(c => c.gabinete === 'G2').map(c => c.doctor))]; return docs.length > 0 ? docs.join(' · ') : 'Gabinete 2'; })()}
                                        </span>
                                    </div>
                                </>) : (
                                    /* Vista single: el gabinete ocupa col 1+2 */
                                    <div style={{
                                        gridColumn: '1 / 3', gridRow: 1,
                                        background: '#fff',
                                        borderBottom: '1px solid #e0e0e0',
                                        ...
                                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#3c4043' }}>
                                            {(() => {
                                                const gab = vistaGabinete as 'G1' | 'G2';
                                                const docs = [...new Set(citas.filter(c => c.gabinete === gab).map(c => c.doctor))];
                                                return docs.length > 0 ? docs.join(' · ') : (gab === 'G1' ? 'Gabinete 1' : 'Gabinete 2');
                                            })()}
                                        </span>
                                    </div>
                                )}
                                ...
                                {/* ═══ BODY ════════════════════════════════════════════════════ */}
                                {timeSlots}
                                {(vistaGabinete === 'ALL' || vistaGabinete === 'G1') && gabCells('G1', g1Col)}
                                {(vistaGabinete === 'ALL' || vistaGabinete === 'G2') && gabCells('G2', g2Col)}

                                {/* ═══ CITAS ═══════════════════════════════════════════════════ */}
                                {(() => {
                                    // Agrupar citas solapadas en wrappers flex para repartir espacio
                                    const visibleCitas = filteredCitas.filter(cita => {
                                        if (cita.gabinete === 'G1' && vistaGabinete === 'G2') return false;
                                        if (cita.gabinete === 'G2' && vistaGabinete === 'G1') return false;
                                        return true;
                                    });
                                    // Renderizar cada cita independientemente — colAssignment gestiona la posición horizontal
                                    return visibleCitas.map(cita => {
                                        const col = cita.gabinete === 'G1' ? g1Col : g2Col;
                                        return renderCita({ ...cita, _gridCol: String(col) } as any, cita.gabinete as 'G1' | 'G2');
                                    });
                                })()}
                            </div>
                        );
